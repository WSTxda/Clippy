...
object BuiltinRulesProvider {
    ...
    val builtinRuleRegexes: List<BuiltinRulesRegex> = listOf(
        ...
        // AliExpress
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?aliexpress\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Amazon
        BuiltinRulesRegex(
            pattern = Regex("^www\\.amazon\\.(com|com\\.br|ca|de|es|fr|it|co\\.uk|co\\.jp)$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("dp|gp")) }),

        // Airbnb
        BuiltinRulesRegex(
            pattern = Regex("^www\\.airbnb\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),
        ...
        // BBC
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?(bbc\\.com|bbc\\.co\\.uk)$"), apply = BuiltinRulesUri::clearQuery
        ),
        ...
        // Bluesky
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?bluesky\\.app$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Booking.com
        BuiltinRulesRegex(
            pattern = Regex("^www\\.booking\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),
        ...
        // Discord
        BuiltinRulesRegex(
            pattern = Regex("^www\\.discord\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // eBay
        BuiltinRulesRegex(
            pattern = Regex("^www\\.ebay\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("id|item_id")) }),

        // Facebook
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?(facebook|m\\.facebook)\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("^(?!fbclid|utm_).*")) }),

        // Google Forms
        BuiltinRulesRegex(
            pattern = Regex("^forms\\.google\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("id|usp")) }),
        ...
        // Imgur
        BuiltinRulesRegex(
            pattern = Regex("^imgur\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),
        ...
        BuiltinRulesRegex(
            ...
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("^(?!li_fat_id|trk|utm_).*")) }),
        ...
        // Medium
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?medium\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Mercado Livre
        BuiltinRulesRegex(
            pattern = Regex("^www\\.mercadolivre\\.com\\.br$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("item_id|id|oid")) }),

        // Notion
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?notion\\.so$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Spotify
        BuiltinRulesRegex(
            pattern = Regex("^(open\\.)?spotify\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("si")) }),
        ...
        // Shein
        BuiltinRulesRegex(
            pattern = Regex("^www\\.shein\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Shopee
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?shopee\\.com(\\.br)?$"), apply = BuiltinRulesUri::clearQuery
        ),

        // SoundCloud
        BuiltinRulesRegex(
            pattern = Regex("^www\\.soundcloud\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),
        ...
        // Substack
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?substack\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Taobao / Tmall
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?(taobao|tmall)\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("id")) }),

        // Telegram
        BuiltinRulesRegex(
            pattern = Regex("^t\\.me$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("start|startapp|text")) }),
        ...
        BuiltinRulesRegex(
            ...
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("ref|adid")) }),

        // Twitter / X
        BuiltinRulesRegex(
            pattern = Regex("^(twitter|x)\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Udemy
        BuiltinRulesRegex(
            pattern = Regex("^(.+\\.)?udemy\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // Vimeo
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?vimeo\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // VK
        BuiltinRulesRegex(
            pattern = Regex("^(www\\.)?vk\\.com$"), apply = BuiltinRulesUri::clearQuery
        ),

        // WhatsApp
        BuiltinRulesRegex(
            pattern = Regex("^api\\.whatsapp\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("phone|text")) }),

        // Wikipedia
        BuiltinRulesRegex(
            pattern = Regex("^www\\.wikipedia\\.org$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("oldid|diff")) }),

        // Yelp
        BuiltinRulesRegex(
            pattern = Regex("^www\\.yelp\\.com$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("ref")) }),

        // YouTube
        BuiltinRulesRegex(
            pattern = Regex("^(youtu\\.be|((www|music)\\.)?youtube\\.com)$"),
            apply = { url -> BuiltinRulesUri.retainParameters(url, Regex("v|list|t|index")) }),
