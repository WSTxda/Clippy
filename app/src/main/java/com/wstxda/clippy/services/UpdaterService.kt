package com.wstxda.clippy.services
...
object UpdaterService {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            try {
                ...
                if (compareVersions(currentVersion, latestVersion) < 0) {
                    showUpdateAvailable(anchorView, latestVersion)
                } else {
                    showNoUpdate(anchorView)
                }
            } catch (_: Exception) {
                showGenericError(anchorView)
            }
        }
    }
    ...
    private fun showNoUpdate(anchorView: View) {
        Snackbar.make(anchorView, R.string.update_checker_no_update, Snackbar.LENGTH_SHORT).show()
    }

    private fun showUpdateAvailable(anchorView: View, latestVersion: String) {
        Snackbar.make(
            anchorView,
    ...
    private fun showNoInternet(anchorView: View) {
        Snackbar.make(anchorView, R.string.update_checker_no_internet, Snackbar.LENGTH_LONG).show()
    }

    private fun showGenericError(anchorView: View) {
